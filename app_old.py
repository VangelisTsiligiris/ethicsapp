    ...
    if st.button("📄 Generate Comprehensive Report", type="primary"):
        report_data = {
            "report_metadata": {
                ...
                "framework_references": [
                    ...
                    "NIST AI RMF 1.0",
                    "UK FCA AI Update 2024",
                    "Singapore MAS FEAT Principles"
                ]
            },
            "risk_assessment": st.session_state.risk_assessment or "Not completed",
            "governance_framework": st.session_state.governance_plan or "Not completed",
            "ethical_assessments": st.session_state.completed_assessments or "None completed"
        }
        ...
        if report_format == "JSON (Machine-readable)":
            ...
            st.download_button(
                ...
                data=report_json,
                file_name=f"comprehensive_ai_ethics_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                mime="application/json"
            )
        else:
            # Generate Markdown report
            md_report = f"""# AI Ethics & Governance Report
...
This report summarizes the AI ethics and governance assessment conducted using the FinTech AI Ethics & Governance Toolkit.
...
{json.dumps(st.session_state.risk_assessment, indent=2, default=str) if st.session_state.risk_assessment else "Risk assessment not completed."}

---

## Governance Framework

{json.dumps(st.session_state.governance_plan, indent=2, default=str) if st.session_state.governance_plan else "Governance framework not created."}

---

## Ethical Assessments

{json.dumps(st.session_state.completed_assessments, indent=2, default=str) if st.session_state.completed_assessments else "No ethical assessments completed."}

---

## Recommendations

Based on the assessments conducted, the following actions are recommended:

1. Address any identified high-priority risks
2. Complete missing governance framework elements
3. Conduct regular re-assessments as regulations evolve
4. Maintain documentation for audit and compliance purposes

---

*Report generated by FinTech AI Ethics & Governance Toolkit*
"""
            st.download_button(
                label="📥 Download Comprehensive Report (Markdown)",
                data=md_report,
                file_name=f"comprehensive_ai_ethics_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
                mime="text/markdown"
            )
